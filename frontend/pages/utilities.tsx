...
import {
  IconDictionary, IconKeyboard, IconPhone, IconSearch, IconCopy, IconX, IconZap, SuccessCheckIcon,
...
const TABS = [
  { id: "dict",     Icon: IconDictionary, label: "Dictionary" },
  { id: "keyboard", Icon: IconKeyboard,   label: "Keyboard" },
  { id: "recharge", Icon: IconPhone,      label: "Recharge" },
] as const;
        ...
        {tab === "dict" && (
          <div className="animate-fadeUp">
            <div className="relative mb-6">
              ...
              <input value={search} onChange={(e) => searchDict(e.target.value)}
                placeholder="Search Nepali or English..."
            ...
            <div className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 overflow-hidden">
              ...
              {!dictLoading && dictResults.map((item: any, i: number) => (
                <div key={item.word_np + i}
                  className="flex items-center justify-between px-6 py-5 border-b border-white/5 hover:bg-white/5 transition-colors animate-fadeUp"
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      {item.category && (
                        <span className="text-[10px] bg-white/10 text-white/40 px-2 py-0.5 rounded-full mt-1 inline-block capitalize">
                          {item.category}
                        </span>
                      )}
                    </div>
                  </div>
                  <button onClick={() => navigator.clipboard?.writeText(item.word_np)}
                    className="text-white/20 hover:text-white transition-colors flex-shrink-0 ml-4"
                    aria-label="Copy">
                    <IconCopy size={16} />
                  </button>
                </div>
          ...
          <div className="animate-fadeUp">
            <div className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 p-6 mb-4">
              <div className="min-h-[80px] backdrop-blur-xl bg-white/5 rounded-2xl border border-white/10 p-4 mb-4 flex items-center justify-between">
                <p className="text-2xl font-devanagari text-white flex-1">
                  {typed || <span className="text-white/20 text-base not-italic">Type using the keyboard below...</span>}
                </p>
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => setTyped(t => t.slice(0, -1))}
                    className="text-white/40 hover:text-white transition-colors px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-sm"
                    aria-label="Backspace">⌫</button>
                  <button onClick={() => navigator.clipboard?.writeText(typed)}
                    className="text-white/40 hover:text-white transition-colors px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-sm"
                    aria-label="Copy">
                    <IconCopy size={14} />
                  </button>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 justify-center mt-2">
                  <button onClick={() => setTyped(t => t + " ")}
                    className="px-12 h-10 rounded-xl backdrop-blur-xl bg-white/5 border border-white/10 text-white/40 hover:bg-white/20 hover:text-white transition-all duration-200 text-xs">
                    SPACE
                  </button>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        { Icon: IconPhone, label: "NTC" },
                        { Icon: IconPhone, label: "Ncell" },
                        { Icon: IconPhone, label: "Smart" },
                      ].map(({ Icon, label }) => (
                        <button key={label} onClick={() => setRechargePhone("")}
                          className="backdrop-blur-xl border rounded-2xl p-4 text-center transition-all duration-200 bg-white/5 border-white/10 hover:bg-white/10">
                  ...
                  <div>
                    ...
                    <input value={rechargePhone} onChange={(e) => setRechargePhone(e.target.value)}
                      ...
                      {[50, 100, 200, 500].map((a) => (
                        <button key={a} onClick={() => setRechargeAmt(String(a))}
                          className={`py-2 rounded-xl text-sm font-bold transition-all duration-200
                            ${rechargeAmt === String(a) ? "bg-white text-gray-900" : "bg-white/5 border border-white/10 text-white hover:bg-white/10"}`}>
                          ₨{a}
                        </button>
                      ))}
                    </div>
                  ...
                  <button onClick={() => rechargePhone && rechargeAmt && setRecharged(true)}
                    disabled={!rechargePhone || !rechargeAmt}
