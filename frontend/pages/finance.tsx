...
import {
  ForexIcon, GoldIcon, SilverIcon, NepseIcon, VegetableIcon, SuryaCoinIcon,
...
const SUMMARY_ICONS = [ForexIcon, GoldIcon, NepseIcon, SilverIcon];
...
export default function Finance() {
  ...
  const summaryCards = [
    { label: "USD/NPR",     value: forex[0]   ? `${forex[0].buy_rate}` : "—",                                  change: "+0.12%" },
    { label: "Gold/Tola",   value: metals[0]  ? `₨${Number(metals[0].price).toLocaleString()}` : "—",          change: metals[0]  ? `${metals[0].change_pct  > 0 ? "+" : ""}${metals[0].change_pct}%`  : "—" },
    { label: "NEPSE",       value: "2,184.32",                                                                  change: "+1.2%" },
    { label: "Silver/Tola", value: metals[2]  ? `₨${Number(metals[2].price).toLocaleString()}` : "—",          change: metals[2]  ? `${metals[2].change_pct}%` : "—" },
  ];
      ...
      <div className="container mx-auto px-4 py-10 max-w-6xl relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-fadeUp">
          {summaryCards.map((s, i) => {
            const CardIcon = SUMMARY_ICONS[i];
            const isPositive = s.change.startsWith("+");
            return (
              <div key={s.label}
                className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 p-5 hover:border-white/25 transition-all duration-300 animate-fadeUp"
                style={{ animationDelay: `${i * 80}ms` }}>
                <div className="flex items-start justify-between mb-3">
                  <CardIcon size={26} color="#F4C430" />
        ...
        {!loading && tab === "Forex" && (
          <div className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 overflow-hidden animate-fadeUp">
            <div className="px-6 py-3 border-b border-white/10 flex items-center gap-3">
              <ForexIcon size={18} color="#F4C430" />
            ...
            <div className="grid grid-cols-4 text-xs text-white/30 uppercase tracking-widest px-6 py-3 border-b border-white/10">
              ...
              <span className="text-right">Buy (NPR)</span>
              <span className="text-right">Sell (NPR)</span>
            </div>
            {forex.map((f, i) => (
              <div key={f.currency_code}
                className="grid grid-cols-4 items-center px-6 py-4 border-b border-white/5 hover:bg-white/5 transition-colors animate-fadeUp"
                style={{ animationDelay: `${i * 25}ms` }}>
                <div className="flex items-center gap-3">
                  {/* flag stays as text — it's a country flag string from backend, not a decorative emoji */}
        ...
        {!loading && tab === "Metals" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 animate-fadeUp">
            {metals.map((m, i) => {
              const MetalIcon = i === 2 ? SilverIcon : GoldIcon;
              const isPositive = Number(m.change_amount) >= 0;
              return (
                <div key={m.metal}
                  className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 p-8 text-center hover:border-aurora-4/40 hover:shadow-[0_0_40px_rgba(245,166,35,0.2)] transition-all duration-500 animate-fadeUp"
                  style={{ animationDelay: `${i * 100}ms` }}>
                  <div className="flex justify-center mb-4 animate-float" style={{ animationDelay: `${i * 0.5}s` }}>
                    <MetalIcon size={64} />
                  </div>
                  ...
                  <span className={`inline-flex items-center gap-1 text-sm font-bold px-3 py-1 rounded-full
                    ...
                    ₨{Math.abs(Number(m.change_amount))} ({Number(m.change_pct) >= 0 ? "+" : ""}{m.change_pct}%)
                  </span>
                </div>
              );
            })}
        ...
        {!loading && tab === "Vegetables" && (
          <div className="animate-fadeUp">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {veggies.map((v, i) => (
                <div key={v.name}
                  className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 p-5 text-center hover:border-green-500/30 transition-all duration-300 animate-fadeUp"
        ...
        {!loading && tab === "Share Market" && (
          <div className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 overflow-hidden animate-fadeUp">
            <div className="px-6 py-3 border-b border-white/10 flex items-center gap-3">
              <NepseIcon size={18} color="#F4C430" />
            ...
            {shares.map((s, i) => {
              const isPositive = Number(s.change_amount) >= 0;
              return (
                <div key={s.symbol}
                  className="grid grid-cols-5 items-center px-6 py-4 border-b border-white/5 hover:bg-white/5 transition-colors animate-fadeUp"
                  ...
                  <span className={`text-right font-mono font-bold flex items-center justify-end gap-1
                    ${isPositive ? "text-green-400" : "text-aurora-1"}`}>
                    {isPositive ? <IconTrendingUp size={12} /> : <IconTrendingDown size={12} />}
                    {Math.abs(Number(s.change_pct))}%
                  </span>
                </div>
              );
            })}
