...
import {
  SuryaCoinIcon, IconCard, IconSend, IconHistory, IconAdd, IconMinus,
  IconZap, IconPhone, IconGlobe, IconCheck, SuccessCheckIcon,
} from "../components/icons";

const SERVICES = [
  { Icon: IconZap,        label: "Electricity",   color: "from-yellow-500/20 to-yellow-900/10 border-yellow-500/20" },
  { Icon: SuryaCoinIcon,  label: "Water",         color: "from-blue-500/20 to-blue-900/10 border-blue-500/20" },
  { Icon: IconPhone,      label: "Mobile Top-up", color: "from-green-500/20 to-green-900/10 border-green-500/20" },
  { Icon: SuryaCoinIcon,  label: "Cable TV",      color: "from-purple-500/20 to-purple-900/10 border-purple-500/20" },
  { Icon: IconGlobe,      label: "Internet",      color: "from-teal-500/20 to-teal-900/10 border-teal-500/20" },
  { Icon: SuryaCoinIcon,  label: "Bank Transfer", color: "from-indigo-500/20 to-indigo-900/10 border-indigo-500/20" },
  { Icon: SuryaCoinIcon,  label: "Insurance",     color: "from-red-500/20 to-red-900/10 border-red-500/20" },
  { Icon: SuryaCoinIcon,  label: "Flight",        color: "from-sky-500/20 to-sky-900/10 border-sky-500/20" },
];

const TRANSACTIONS = [
  { Icon: IconZap,       label: "NEA Electricity",     amount: -1450, date: "Today, 10:32 AM",    status: "success" },
  { Icon: IconPhone,     label: "NTC Top-up",           amount: -500,  date: "Yesterday, 3:15 PM", status: "success" },
  { Icon: SuryaCoinIcon, label: "Received from Hari",   amount: +2000, date: "Dec 12, 9:00 AM",   status: "success" },
  { Icon: IconGlobe,     label: "WorldLink Internet",   amount: -1200, date: "Dec 10, 11:45 AM",  status: "success" },
  { Icon: SuryaCoinIcon, label: "Bank Withdrawal",      amount: -5000, date: "Dec 8, 2:30 PM",    status: "success" },
];

const TABS = [
  { id: "home",    Icon: SuryaCoinIcon, label: "Services" },
  { id: "pay",     Icon: IconCard,      label: "Pay Bill" },
  { id: "send",    Icon: IconSend,      label: "Send Money" },
  ...
  return (
    <div className="min-h-screen bg-aurora relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-10 max-w-4xl relative z-10">
        ...
        <div className="relative backdrop-blur-2xl bg-gradient-to-br from-green-500/20 to-teal-900/20 rounded-3xl border border-green-500/20 p-8 mb-6 overflow-hidden animate-fadeUp">
          ...
          <div className="relative">
            ...
            <div className="flex gap-3 mt-6">
              {[
                { Icon: IconAdd,       label: "Add Money" },
                { Icon: IconMinus,     label: "Withdraw" },
                { Icon: IconSend,      label: "Send" },
                { Icon: SuryaCoinIcon, label: "Request" },
              ].map(({ Icon, label }) => (
                <button key={label}
                  className="flex flex-col items-center gap-1 backdrop-blur-xl bg-white/10 border border-white/10 rounded-2xl px-4 py-3 hover:bg-white/20 transition-all duration-200 hover:scale-105">
                  <Icon size={20} color="#F4C430" />
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-6 animate-fadeUp flex-wrap">
          ...
          <div className="max-w-md mx-auto backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 p-8 animate-fadeUp">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={billAccount} onChange={(e) => setBillAccount(e.target.value)}
              ...
              {!billPaid ? (
                <button onClick={() => billAccount && amount && setBillPaid(true)}
                  disabled={!billAccount || !amount}
        ...
        {tab === "history" && (
          <div className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/10 overflow-hidden animate-fadeUp">
            {TRANSACTIONS.map((t, i) => (
              <div key={i}
                className="flex items-center gap-4 px-6 py-5 border-b border-white/5 hover:bg-white/5 transition-colors animate-fadeUp"
                style={{ animationDelay: `${i * 50}ms` }}>
                <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center flex-shrink-0">
                  <t.Icon size={22} color="#F4C430" />
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center justify-end gap-1 text-[10px] text-green-400/70">
                    <IconCheck size={10} />
                    <span>{t.status}</span>
                  </div>
                </div>
