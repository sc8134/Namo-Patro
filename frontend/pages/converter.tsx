...
import { ConverterArrowIcon, IconCopy, IconWarning, IconLoader } from "../components/icons";
    ...
    <div className="min-h-screen bg-aurora relative overflow-hidden flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12 relative z-10">
        <div className="w-full max-w-2xl">
            ...
            <div className="relative backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-1.5 flex gap-1">
              {(["ad2bs", "bs2ad"] as const).map((m) => (
                <button key={m} onClick={() => { setMode(m); setResult(null); setError(""); }}
                  ...
                  <span className="relative">{m === "ad2bs" ? "AD → BS" : "BS → AD"}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="backdrop-blur-2xl bg-white/5 rounded-3xl border border-white/15 p-8 mb-6 animate-fadeUp" style={{ animationDelay: "200ms" }}>
            {mode === "ad2bs" ? (
              <div className="space-y-4">
                <label className="block text-xs text-white/40 uppercase tracking-[0.2em] mb-2">Enter AD Date</label>
                <input type="date" value={adDate} onChange={(e) => setAdDate(e.target.value)}
                  className="w-full backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white text-lg font-mono placeholder-white/20 outline-none focus:border-white/30 focus:bg-white/10 transition-all duration-300 [color-scheme:dark]" />
              </div>
            ) : (
              <div className="space-y-4">
                <label className="block text-xs text-white/40 uppercase tracking-[0.2em] mb-2">Enter BS Date</label>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input type="number" placeholder="2081" value={bsYear} onChange={(e) => setBsYear(e.target.value)}
                      className="w-full backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl px-4 py-4 text-white text-lg font-mono placeholder-white/20 outline-none focus:border-white/30 focus:bg-white/10 transition-all duration-300" />
                  </div>
                  <div>
                    <p className="text-xs text-white/30 mb-2">Month</p>
                    <select value={bsMonth} onChange={(e) => setBsMonth(e.target.value)}
                      className="w-full backdrop-blur-xl bg-[#1a0a2e] border border-white/10 rounded-2xl px-4 py-4 text-white text-sm outline-none focus:border-white/30 transition-all duration-300">
                      {BS_MONTHS.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
                    </select>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 mb-2">Day</p>
                    <input type="number" placeholder="1" min="1" max="32" value={bsDay} onChange={(e) => setBsDay(e.target.value)}
                      className="w-full backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl px-4 py-4 text-white text-lg font-mono placeholder-white/20 outline-none focus:border-white/30 focus:bg-white/10 transition-all duration-300" />
                  </div>
                </div>
              </div>
            ...
            <button onClick={convert}
              ...
              className="mt-6 w-full relative group overflow-hidden rounded-2xl py-4 font-bold text-white transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-[0.98]">
              ...
              <span className="relative flex items-center justify-center gap-2 text-lg">
                {loading ? (
                  ...
                  <>
                    <ConverterArrowIcon size={22} color="#fff" />
          ...
          {/* Error */}
          {error && (
            <div className="backdrop-blur-xl bg-aurora-1/10 border border-aurora-1/30 rounded-2xl p-4 flex items-center gap-3 text-white/70 text-sm animate-fadeUp">
              <IconWarning size={18} className="text-aurora-1 flex-shrink-0" />
              {error}
            </div>
          )}
              ...
              <div className="relative">
                ...
                <div className="mt-4 flex items-center justify-center gap-3">
                  ...
                  <button onClick={() => navigator.clipboard?.writeText(result || "")}
                    className="flex items-center gap-1.5 backdrop-blur-xl bg-white/10 border border-white/10 hover:bg-white/20 text-white/60 hover:text-white px-3 py-2 rounded-xl text-xs transition-all duration-200">
                    <IconCopy size={13} />
                    Copy
                  </button>
                </div>
