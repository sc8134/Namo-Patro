...
import { VideoCallIcon, IconLock, IconTimer, IconUsers, IconCopy, IconX } from "../components/icons";
...
export default function Meet() {
  ...
  useEffect(() => {
    ...
    return () => { apiRef.current?.dispose(); document.head.removeChild(script); };
  }, [joined, roomId, name]);
    ...
    return (
      <div className="min-h-screen bg-aurora flex flex-col">
        ...
        <div className="flex-1 flex flex-col container mx-auto px-4 py-4 max-w-6xl relative z-10">
          ...
          <div className="flex items-center gap-4 mb-4 backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl px-5 py-3 animate-fadeUp">
            ...
            <div className="flex items-center gap-2 ml-4">
              ...
              <button onClick={copyRoomId}
                className="flex items-center gap-1 text-xs bg-white/10 hover:bg-white/20 text-white/60 hover:text-white px-2 py-1 rounded-lg transition-all">
                <IconCopy size={12} />
                {copied ? "Copied" : "Copy"}
              </button>
            </div>
            <button onClick={() => { setJoined(false); apiRef.current?.dispose(); }}
              className="ml-auto flex items-center gap-1 text-xs bg-red-500/20 text-red-400 border border-red-500/30 px-4 py-1.5 rounded-xl hover:bg-red-500/30 transition-all">
              <IconX size={12} />
              Leave
            </button>
          </div>
          ...
          {/* Jitsi container */}
          <div ref={jitsiRef}
            className="flex-1 rounded-3xl overflow-hidden border border-white/10 bg-black/40 min-h-[500px]">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12 relative z-10">
        <div className="w-full max-w-lg">
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleStart}
                ...
                <span className="relative flex items-center justify-center gap-2 text-lg">
                  <VideoCallIcon size={20} color="#fff" />
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 animate-fadeUp" style={{ animationDelay: "200ms" }}>
            {[
              { Icon: IconLock,   title: "Encrypted", desc: "End-to-end secure" },
              { Icon: IconTimer,  title: "No Limits",  desc: "Unlimited duration" },
              { Icon: IconUsers,  title: "100+",       desc: "Participants" },
            ].map(({ Icon, title, desc }) => (
              <div key={title} className="backdrop-blur-xl bg-white/5 rounded-2xl border border-white/10 p-4 text-center">
                <div className="flex justify-center mb-2">
                  <Icon size={24} color="#F4C430" />
                </div>
